...
contract EvoSouls is ERC721, ERC721URIStorage, Ownable {
    ...
    // Mint price (FREE for testnet)
    uint256 public constant MINT_PRICE = 0; // FREE for testing
    
    constructor(string memory _name, string memory _symbol, string memory _baseURI) 
        ERC721(_name, _symbol) 
        Ownable(msg.sender) // For OpenZeppelin v5, pass initial owner
    {
        baseTokenURI = _baseURI;
    }
    ...
    function mintCreature(
        ...
        // Commented out for testnet - make it FREE
        // require(msg.value >= MINT_PRICE, "Insufficient payment");
    ...
    function setTokenURI(uint256 _tokenId, string memory _tokenURI) public {
        require(_ownerOf(_tokenId) == msg.sender || owner() == msg.sender, "Not authorized");
        _setTokenURI(_tokenId, _tokenURI);
    }
    
    function getCreature(uint256 _tokenId) public view returns (Creature memory) {
        require(_tokenId < _tokenIdCounter.current(), "Creature does not exist");
        return creatures[_tokenId];
    }
